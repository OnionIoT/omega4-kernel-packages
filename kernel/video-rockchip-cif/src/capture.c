			     ...
			     u32 pad_id, struct csi_channel_info *csi_info)
{
	...
	ret_get_fmt = v4l2_subdev_call(sd, pad, get_fmt, NULL, &fmt);
	if (ret_get_fmt < 0) {
		v4l2_err(sd->v4l2_dev,
			 "get_fmt failed: %d (pad %u) sd=%s which=%u fmt_pad=%u\n",
			 ret_get_fmt, pad_id, sd->name, fmt.which, fmt.pad);
		v4l2_warn(sd->v4l2_dev,
			  "get_fmt failed, trying RKMODULE_GET_CHANNEL_INFO\n");
	}
	...
	} else if (ret_get_fmt < 0) {
		v4l2_warn(sd->v4l2_dev,
			  "sensor fmt invalid, set to default size\n");
		goto set_default;
	} else {
		v4l2_warn(sd->v4l2_dev,
			  "get_channel_info failed: %d (pad %u)\n",
			  ret, pad_id);
		csi_info->vc = pad_id;
	}
...
set_default:
	...
	if (sd && sd->name && strstr(sd->name, "sc3336")) {
		for (i = 0; i < ARRAY_SIZE(in_fmts); i++)
			if (in_fmts[i].mbus_code == MEDIA_BUS_FMT_SBGGR10_1X10 &&
			    in_fmts[i].field == V4L2_FIELD_NONE)
				return &in_fmts[i];
	}
				  ...
				  struct csi_channel_info *channel)
{
	...
	if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact &&
		...
		/*
		 * RV1103B/RK3576 CSI size check uses the programmed line width.
		 * Padding the stride to 256 bytes can trigger CSI_SIZE_ERR, so keep
		 * RAW stride tightly aligned.
		 */
		if (dev->chip_id >= CHIP_RK3576_CIF)
			raw_align = 8;

		if (channel->capture_info.mode == RKMODULE_MULTI_DEV_COMBINE_ONE) {
			channel->virtual_width = ALIGN(channel->width * 2 * fmt->raw_bpp / 8, raw_align);
			channel->left_virtual_width = channel->width * fmt->raw_bpp / 8;
		} else {
			channel->virtual_width = ALIGN(channel->width * fmt->raw_bpp / 8, raw_align);
		}
	} else {
		if (channel->capture_info.mode == RKMODULE_MULTI_DEV_COMBINE_ONE) {
...
void rkcif_reinit_right_half_config(struct rkcif_stream *stream)
{
	...
	channel->width /= 2;
	channel->width += RKMOUDLE_UNITE_EXTEND_PIXEL;
	channel->virtual_width = ALIGN(channel->width * fmt->raw_bpp / 8,
				       cif_dev->chip_id >= CHIP_RK3576_CIF ? 8 : 256);
...
static u32 rkcif_get_right_half_buf_size(struct rkcif_stream *stream)
{
	...
	if (stream->is_compact)
		virtual_width = ALIGN(width * stream->cif_fmt_out->raw_bpp / 8,
				      stream->cifdev->chip_id >= CHIP_RK3576_CIF ? 8 : 256);
	else
		  ...
		  bool try)
{
	...
	if (dev->terminal_sensor.sd) {
		...
		if (!cif_fmt_in) {
			v4l2_err(&stream->cifdev->v4l2_dev,
				 "terminal sensor input format invalid\n");
			return -EINVAL;
		}
	...
	for (i = 0; i < planes; i++) {
		...
		if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact &&
		    (dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY ||
		     ...
		     fmt->csi_fmt_val != CSI_WRDDR_TYPE_RGB888 &&
		     fmt->csi_fmt_val != CSI_WRDDR_TYPE_RGB565) {
			bpl = ALIGN(width * fmt->raw_bpp / 8,
				    dev->chip_id >= CHIP_RK3576_CIF ? 8 : 256);
		} else {
			if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact &&
			    fmt->csi_fmt_val != CSI_WRDDR_TYPE_RGB888 &&
			    fmt->csi_fmt_val != CSI_WRDDR_TYPE_RGB565 &&
					    ...
					    u32 *crop_vwidth)
{
	...
	*crop_width = raw_width;
	...
	if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact &&
		...
		/*
		 * RV1103B/RK3576 CSI size check uses the programmed line width.
		 * Keep RAW stride tightly aligned to avoid CSI_SIZE_ERR.
		 */
		if (dev->chip_id >= CHIP_RK3576_CIF)
			raw_align = 8;

		*crop_vwidth = ALIGN(raw_width * fmt->raw_bpp / 8, raw_align);
	} else {
...
void rkcif_err_print_work(struct work_struct *work)
{
	...
	if (err_state & RKCIF_ERR_SIZE && is_print) {
		u32 set_size_id0 = 0;
		u32 vlw_id0 = 0;
		u32 ctrl0_id0 = 0;
		u32 effect_id0 = 0;
		...
		/*
		 * On RK3576/RV1103B the "FRAME_SIZE_ID*" registers are not mapped
		 * in our reg table, so print SET_SIZE + key CSI registers instead.
		 */
		if (dev->chip_id >= CHIP_RK3576_CIF)
			set_size_id0 = rkcif_read_register(dev, CIF_REG_MIPI_SET_SIZE_ID0);
		else if (dev->chip_id >= CHIP_RK3588_CIF)
			set_size_id0 = err_state_work->size_id0;

		vlw_id0 = rkcif_read_register(dev, get_reg_index_of_frm0_y_vlw(0));
		ctrl0_id0 = rkcif_read_register(dev, get_reg_index_of_id_ctrl0(0));
		effect_id0 = rkcif_read_register(dev, CIF_REG_MIPI_EFFECT_CODE_ID0);
		on_pad = rkcif_read_register(dev, CIF_REG_MIPI_ON_PAD);

		v4l2_err(&dev->v4l2_dev,
			 "ERROR: csi size err, intstat:0x%x, lastline:0x%x, set_size_id0:0x%x, vlw_id0:0x%x, id0_ctrl0:0x%x, effect_id0:0x%x, on_pad:0x%x, cnt %llu\n",
			 intstat, lastline, set_size_id0, vlw_id0, ctrl0_id0, effect_id0, on_pad,
