...
static int rkvenc_run(struct mpp_dev *mpp, struct mpp_task *mpp_task)
{
	 ...
	 */
	core_rate = mpp_get_clk_info_rate_hz(&enc->core_clk_info, task->clk_mode);
	if (!core_rate)
		core_rate = clk_get_rate(enc->core_clk_info.clk);
	if (!core_rate) {
		core_rate = 400 * MHZ;
		dev_warn_once(mpp->dev, "using fallback rkvenc watchdog clock %lu\n",
			      core_rate);
	}
	timeout_thd = mpp_read(mpp, RKVENC_WDG);
	timeout_thd = (timeout_thd & 0xff000000) |
		      max(timeout_thd & 0x00ffffff,
			  (u32)(TIMEOUT_MS * core_rate / 1024000));
