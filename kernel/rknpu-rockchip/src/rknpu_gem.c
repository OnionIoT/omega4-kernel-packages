...
#define RKNPU_IOMMU_MAP(domain, iova, paddr, size, prot) \
		...
		if (dir & RKNPU_MEM_SYNC_TO_DEVICE) {
			...
			dcache_clean_poc((unsigned long)cache_start,
					 (unsigned long)cache_start +
						 cache_length);
#else
			flush_kernel_vmap_range((void *)cache_start,
						cache_length);
#endif
			}
			...
			dcache_inval_poc((unsigned long)cache_start,
					 (unsigned long)cache_start +
						 cache_length);
#else
			invalidate_kernel_vmap_range((void *)cache_start,
						     cache_length);
#endif
			}
