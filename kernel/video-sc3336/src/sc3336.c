				...
				struct v4l2_mbus_config *config)
{
	...
	config->bus.mipi_csi2.flags = BIT(SC3336_LANES - 1);
...
static int sc3336_get_channel_info(struct sc3336 *sc3336,
				   struct rkmodule_channel_info *ch_info)
{
	...
	ch_info->vc = 0;
	ch_info->width = sc3336->cur_mode->width;
	ch_info->height = sc3336->cur_mode->height;
	ch_info->bus_fmt = sc3336->cur_mode->bus_fmt;
	ch_info->data_type = 0;
	ch_info->data_bit = SC3336_BITS_PER_SAMPLE;
...
static long sc3336_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_CHANNEL_INFO:
		...
		ret = sc3336_get_channel_info(sc3336, ch_info);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case RKMODULE_GET_CHANNEL_INFO:
		...
		ret = sc3336_ioctl(sd, cmd, ch_info);
		if (!ret) {
			if (copy_to_user(up, ch_info, sizeof(*ch_info)))
				ret = -EFAULT;
		}
		kfree(ch_info);
		break;
