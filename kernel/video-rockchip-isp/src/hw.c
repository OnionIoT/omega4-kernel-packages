...
static int rkisp_hw_probe(struct platform_device *pdev)
{
	...
	/* ISP MI uses linear DMA. Without an IOMMU, buffers must be contiguous. */
	...
	/*
	 * If an IOMMU is present, SG is fine (IOMMU can translate a linear IOVA
	 * even when backing pages are scattered). Without IOMMU, keep buffers
	 * physically contiguous so the MI doesn't walk off into unrelated RAM.
	 */
	if (hw_dev->is_mmu) {
		...
		hw_dev->is_dma_sg_ops = false;
		hw_dev->mem_ops = &vb2_cma_contig_memops;
	}
