...
static int rkisp_stream_config_rsz(struct rkisp_stream *stream, bool async)
{
	...
	if (rkisp_mbus_code_xysubs(input_isp_fmt->mbus_code,
				  &xsubs_in, &ysubs_in)) {
	...
	if (output_isp_fmt->fmt_type == FMT_YUV) {
		rkisp_fcc_xysubs(output_isp_fmt->fourcc,
				 &xsubs_out, &ysubs_out);
...
static dma_addr_t rkisp_sg_dma_addr_offset(struct sg_table *sgt, size_t offset)
{
...
static void rkisp_buf_queue(struct vb2_buffer *vb)
{
	...
	if (isp_fmt->mplanes == 1) {
		for (i = 0; i < isp_fmt->cplanes - 1; i++) {
			...
			if (stream->ispdev->hw_dev->is_dma_sg_ops && sgt)
				ispbuf->buff_addr[i + 1] =
					rkisp_sg_dma_addr_offset(sgt, offset);
			else
				ispbuf->buff_addr[i + 1] =
