...
EXPORT_SYMBOL(rkcif_csi2_register_notifier);
EXPORT_SYMBOL(rkcif_csi2_unregister_notifier);
			    ...
			    struct v4l2_subdev_format *fmt)
{
	...
	if (ret) {
		v4l2_warn(&csi2->sd,
			  "get_fmt failed, fallback to RKMODULE_GET_CHANNEL_INFO\n");
		ch_info.index = fmt->pad;
		ret = v4l2_subdev_call(sensor, core, ioctl,
				       RKMODULE_GET_CHANNEL_INFO, &ch_info);
		if (ret)
			return ret;
		fmt->format.width = ch_info.width;
		fmt->format.height = ch_info.height;
		fmt->format.code = ch_info.bus_fmt;
		fmt->format.field = ch_info.field ? ch_info.field : V4L2_FIELD_NONE;
	}
			...
			if (ret) {
				...
				if (!ret) {
					...
					ret = v4l2_subdev_call(sensor, core, ioctl,
							       RKMODULE_GET_CHANNEL_INFO, &ch_info);
					if (!ret) {
						csi2->format_mbus.width = ch_info.width;
						csi2->format_mbus.height = ch_info.height;
						csi2->format_mbus.code = ch_info.bus_fmt;
						csi2->format_mbus.field = ch_info.field ?
...
EXPORT_SYMBOL(rkcif_csi2_event_reset_pipe);
EXPORT_SYMBOL(rkcif_csi2_event_inc_sof);
EXPORT_SYMBOL(rkcif_csi2_get_sof);
EXPORT_SYMBOL(rkcif_csi2_set_sof);
...
static int __init rockchip_mipi_csi2_init(void)
{
	int ret;

	ret = rkcif_csi2_hw_plat_drv_init();
	if (ret)
		return ret;

	ret = rkcif_csi2_plat_drv_init();
	if (ret) {
...
static void __exit rockchip_mipi_csi2_exit(void)
{
	rkcif_csi2_plat_drv_exit();
	rkcif_csi2_hw_plat_drv_exit();
}

module_init(rockchip_mipi_csi2_init);
module_exit(rockchip_mipi_csi2_exit);
